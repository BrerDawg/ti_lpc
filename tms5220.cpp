...
char fullname[] = { "tmc0351n2l.vsm\0" };

InFile = fopen( fullname , "rb" );
if (InFile!=NULL)
	{
	fread( phrom_rom[ 0 ], 1, 16384, InFile );
	fclose(InFile);
	}
else{
	SpeechError("Cannot open specified PHROM: %s\n", fullname );
...
printf("phrom: %04x count: %d\n",tms5220->chip->phrom_address & 0x3fff, count );
...
//	fprintf(stderr, "In my_load_address with data = 0x%02x, new address = 0x%05x\n", data, tms5220->chip->phrom_address);
	printf("my_load_address() - 0x%02x, new address: 0x%05x\n", data, tms5220->chip->phrom_address);
}
...
tms5220->chip->period_use_5_bit = 0;
...
printf("tms5220_update() - here0\n");
	/* finish off the current sample */
	if (info->source_pos > 0)
	{
printf("tms5220_update() - here1\n");
...
//			fprintf(stderr, "Sample = %d\n", samp);
			*buffer++ = samp;
//printf("tms5220_update() - sample: %d\n", samp );
...
//			fprintf(stderr, "Sample = %d\n", samp);
			*buffer++ = samp;
//printf("tms5220_update() - samplehere: %d\n", samp );
			info->source_pos += info->source_step;
			length--;
		}
...
//				bfchirp[ buf_count ] = chirptable[tms->pitch_count];
				}
            else{
                current_val = 0x00;
				}
...
static int parse_frame(struct tms5220 *tms, int the_first_frame)
{
	...
	if( tms->period_use_5_bit )
		{
		indx = extract_bits(tms, 5);							//6 for tms5220, 5 for older tms5100
		}
	else{
		indx = extract_bits(tms, 6);							//6 for tms5220, 5 for older tms5100
		}
...
static void check_buffer_low(struct tms5220 *tms)
{
printf( "check_buffer_low() - tms->fifo_count: %d\n", tms->fifo_count);
